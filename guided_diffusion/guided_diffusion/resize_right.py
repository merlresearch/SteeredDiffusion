...
# Code adapted  from https://github.com/assafshocher/ResizeRight/blob/master/resize_right.py -- MIT License
...
from .interp_methods import box, cubic, lanczos2, lanczos3, linear, support_sz
...
class Resizer(nn.Module):
    def __init__(
        ...
        interp_method = interpmethods[interp_method]
        ...
        scale_factors, out_shape, by_convs = self.set_scale_and_out_sz(
        ...
        output = input
        self.projected_grids = []
        self.cur_interp_methods = []
        self.cur_support_szs = []
        self.fov = []
        self.weights = []
        self.pad_sz = []
        self.projected_grid = []
        ...
        for (dim, scale_factor, dim_by_convs, in_sz, out_sz) in self.sorted_filtered_dims_and_scales:
            ...
            self.dim_by_convs.append(dim_by_convs)
            self.in_sz.append(in_sz)
            self.out_sz.append(out_sz)
            # STEP 1- PROJECTED GRID: The non-integer locations of the projection
            # of output pixel locations to the input tensor
            projected_grid = self.get_projected_grid(in_sz, out_sz, scale_factor, fw, dim_by_convs, device)
            ...
            cur_interp_method, cur_support_sz = self.apply_antialiasing_if_needed(
                interp_method, support_sz, scale_factor, antialiasing
            )
            self.cur_interp_methods.append(cur_interp_method)
            self.cur_support_szs.append(cur_support_sz)
            ...
            # accoedingly
            field_of_view = self.get_field_of_view(projected_grid, cur_support_sz, fw, eps, device)
            ...
            pad_sz, projected_grid, field_of_view = self.calc_pad_sz(
                in_sz, out_sz, field_of_view, projected_grid, scale_factor, dim_by_convs, fw, device
            )
            self.pad_sz.append(pad_sz)
            self.projected_grids.append(projected_grid)
            self.fov.append(field_of_view)

            # STEP 3- CALCULATE WEIGHTS: Match a set of weights to the pixels in
            # the field of view for each output pixel
            weights = self.get_weights(cur_interp_method, projected_grid, field_of_view)
    ...
    def forward(self, input):
        _, n_dims = input.shape, input.ndim
        for i in range(len(self.dim)):
            field_of_view = self.fov[i]
            weights = self.weights[i]
            dim = self.dim[i]
            scale_factor = self.scale_factor[i]
            dim_by_convs = self.dim_by_convs[i]
            in_sz = self.in_sz[i]
            out_sz = self.out_sz[i]
            pad_sz = self.pad_sz[i]

        if not dim_by_convs:
            output = self.apply_weights(input, field_of_view, weights, dim, n_dims, pad_sz, self.pad_mode, self.fw)
        else:
            output = self.apply_convs(input, scale_factor, in_sz, out_sz, weights, dim, pad_sz, self.pad_mode, self.fw)
        return output

    def get_projected_grid(self, in_sz, out_sz, scale_factor, fw, by_convs, device=None):
    ...
    def get_field_of_view(self, projected_grid, cur_support_sz, fw, eps, device):
    ...
    def calc_pad_sz(self, in_sz, out_sz, field_of_view, projected_grid, scale_factor, dim_by_convs, fw, device):
        if not dim_by_convs:
            ...
            right_pads = (
                ((out_sz - fw_arange(num_convs, fw, device) - 1) // num_convs) * stride  # (1)  # (2)  # (3)
                + field_of_view[:, -1]  # (4)
                - in_sz
                + 1
            )  # (5)
    ...
    def get_weights(self, interp_method, projected_grid, field_of_view):
    ...
    def apply_weights(self, input, field_of_view, weights, dim, n_dims, pad_sz, pad_mode, fw):
    ...
    def apply_convs(self, input, scale_factor, in_sz, out_sz, weights, dim, pad_sz, pad_mode, fw):
    ...
    def set_scale_and_out_sz(
        self, in_shape, out_shape, scale_factors, by_convs, scale_tolerance, max_numerator, eps, fw
    ):
        ...
        if scale_factors is not None:
            ...
            scale_factors = (
                scale_factors if isinstance(scale_factors, (list, tuple)) else [scale_factors, scale_factors]
            )
    ...
    def apply_antialiasing_if_needed(self, interp_method, support_sz, scale_factor, antialiasing):
